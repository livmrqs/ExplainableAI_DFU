...
# ============================================================
# 5. LIME DIRECTLY ON THE ORIGINAL IMAGE
# ============================================================
#
# What this script does:
#   - Trains PCA + XGBoost using the same pipeline as your original code.
#   - Applies LIME Image directly to the original image.
#   - LIME perturbs image superpixels, and each perturbed image passes through:
#         image -> flatten -> PCA -> XGBoost -> probabilities
#   - Saves CAM-like figures:
#         Original | Heatmap | Overlay
#   - Also saves the superpixel boundary image.
#
# Expected output:
#   ../reports/xai/ischaemia_xgb_pca/5_lime_image_direct/
#       lime_image_0_TP_...png
#       lime_image_boundaries_0_TP_...png
#       ...
#
# Interpretation:
#   - This is the most visually similar to CAM explanations, but it is slower.
#   - Red/yellow regions represent superpixels whose perturbation most affected
#     the final PCA + XGBoost prediction.

from lime import lime_image
from skimage.segmentation import mark_boundaries

xai_dir = f"../reports/xai/{task_name}_xgb_pca"
lime_img_dir = os.path.join(xai_dir, "5_lime_image_direct")
os.makedirs(lime_img_dir, exist_ok=True)

print("\n[5] Running LIME directly on original images...")


def predict_fn_lime_image(images):
    """
    Prediction function used by LIME Image.

    Input:
        images: array with shape (n_samples, height, width, channels)

    Output:
        probabilities: array with shape (n_samples, 2)
    """
    images = np.array(images)
    images = np.clip(images, 0, 1)

    images_flat = images.reshape(images.shape[0], -1)
    images_pca = pca_final.transform(images_flat)

    probs = clf_final.predict_proba(images_pca)
...
for idx in range(min(num_samples_to_explain, len(X_test_pca))):
    ...
    explanation = lime_image_explainer.explain_instance(
        image=original_img,
        classifier_fn=predict_fn_lime_image,
        top_labels=2,
        hide_color=0,
        num_samples=300
    )
    ...
    # CAM-like heatmap from LIME's selected superpixel mask
    heatmap = mask.astype(float)
    heatmap = normalize_heatmap(heatmap)

    title = (
        f"Method=LIME direct image | Group={group} | "
    ...
    save_path = os.path.join(
        lime_img_dir,
        f"lime_image_{idx}_{group}_{image_name}_true{true_label}_pred{pred_label}.png"
    )
    ...
    lime_boundary_img = mark_boundaries(temp, mask)

    plt.figure(figsize=(6, 6))
    plt.imshow(lime_boundary_img)
    plt.title(title)
    plt.axis("off")

    boundary_path = os.path.join(
        lime_img_dir,
        f"lime_image_boundaries_{idx}_{group}_{image_name}_true{true_label}_pred{pred_label}.png"
    )

    plt.savefig(boundary_path, dpi=300, bbox_inches="tight")
    plt.close()

print(f"\n[Done] LIME direct image outputs saved in: {lime_img_dir}")
